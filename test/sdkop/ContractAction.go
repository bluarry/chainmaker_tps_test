...
func createUserContract(client *sdk.ChainClient, admins []*sdk.ChainClient,
	...
	signedPayloadBytes :=make([][]byte,0)
	// 各组织Admin权限用户签名
	for i:=0;i<len(admins);i++{
		fmt.Printf("admin%d 签名完了\n",i)
		signedPayloadBytes1, err := admins[i].SignContractManagePayload(payloadBytes)
		if err != nil {
			return nil, err
		}
		signedPayloadBytes=append(signedPayloadBytes,signedPayloadBytes1);
	}
...
func userContractAssetCreate(client *sdk.ChainClient,
	admins []*sdk.ChainClient, kvs []*common.KeyValuePair, withSyncResult bool, isIgnoreSameContract bool)error {

	resp, err := createUserContract(client, admins,
