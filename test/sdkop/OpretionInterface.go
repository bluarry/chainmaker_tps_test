...
func ContractInstance()error{
	...
	Client1=chainClient
	//fmt.Printf("%v\n",chainClient)
	admins:=make([]*sdk.ChainClient,0)

	for i:=1;i<=16;i++{
		fmt.Printf("#====================== create admin%d ======================\n",i)
		admin, err := createAdminWithConfig(fmt.Sprintf("node%d",i))
		if err!=nil{
			// fmt.Println(err)
			return err
		}
	...
	err=userContractAssetCreate(chainClient, admins, pairs, true, true)
	if err!=nil{
		return err
	}
...
func RegisterUser(){
	...
	userContractAssetInvokeRegister(client2, "register", false)
